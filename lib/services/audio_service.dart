...
class AudioService {
  ...
  AudioService._internal() {
    ...
    player.playerStateStream.listen((state) {
      if (state.processingState == ProcessingState.completed) {
        if (loopMode.value == LoopMode.off && _currentIndex == _queue.length - 1) {
          player.stop();
        }
      }
    });
  ...
  Future<AudioSource> _createAudioSource(Video video) async {
    ...
    if (await localFile.exists() && (await localFile.length()) > 10000) {
      ...
      // 2. Fetch Online (MUXED ONLY as requested)
      try {
        ...
        if (manifest.muxed.isNotEmpty) {
           ...
           print("⚠️ No Muxed streams available for: ${video.title}");
        }
      } catch (e) {
        print("❌ Stream Error for ${video.title}: $e");
      }
    }

    // 3. Safety Fallback (Prevents "Song Can't Load" crash)
    if (url.isEmpty) {
        return AudioSource.uri(
            Uri.parse("https://www.soundhelix.com/examples/mp3/SoundHelix-Song-1.mp3"), // Silent/Dummy URL
            tag: MediaItem(
                id: video.id.value,
                album: "Error",
  ...
  Future<void> _fetchLyrics(String rawTitle, String rawArtist, String description) async {
    currentSongLyrics.value = "Searching for lyrics...\n\n(Fallback: Description)\n$description";
